...
if(os.platform() == 'win32')
{
  ...
  process.env.PATH = process.env.PATH + ';' +
                     ...
                     path.resolve(__dirname, '../../../../app.asar.unpacked/node_modules/ibm_db/installer/clidriver/bin') + ';' +
                     path.resolve(__dirname, '../../../../app.asar.unpacked/node_modules/ibm_db/installer/clidriver/lib');
...
Database.prototype.checkConnectionError = function (err)
{
  ...
  if(self.realClose){
    if((err && err['message'] && err['message'].search("SQL30081N") != -1))
    {
      ...
      if (self.availablePool && self.availablePool.length) {
    ...
    fs.readFile(sql, function (err, sql) {
        if (err) {
            ...
            for (var i = 0; i < myarray.length; i++) {
                ...
                if (!Array.isArray(result)) {
                    if (outputfile === undefined) {
                        deferred ? deferred.reject(err) : cb(result);
                    }
                    else {
            ...
            deferred ? deferred.reject(err) : cb(err, res);
        }
    });
}; 
